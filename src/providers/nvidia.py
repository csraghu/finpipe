...
class NvidiaAdapter(ILLMProvider):
    ...
    async def generate_response(
        ...
        cache_key = f"nvidia_{model_name}_{hash(prompt)}"
        ...
        try:
            response = await self._client.request(
                "POST", self._chat_url, headers=headers, json=payload
            )
            data = response.json()
        except Exception as exc:
            logger.error("NVIDIA API request failed: %s", exc)
            raise FinpipeProviderDownError("Failed to communicate with NVIDIA API") from exc

        choices = data.get("choices", [])
        if not choices:
            raise FinpipeProviderDownError("NVIDIA API returned an empty response")
...
@register_provider("nvidia", category="llm")
def build_nvidia(ctx: BuildContext) -> NvidiaAdapter:
    return NvidiaAdapter(ctx.config)
